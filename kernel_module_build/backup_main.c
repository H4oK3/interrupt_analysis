...
{   
    ...
    DBG("name: %s, pid: [%d], state: %li pgdir=%08x\n", task->comm, task->pid, task->state,
            (task->mm ? task->mm->pgd : 0));
    int n = 0;
...
#define DUMPSIZE 0x1000
int init_module (void)
{
    int i;
    DBG("=== START ===");
    if(current->mm) DBG("Current task: %p pgd=%x", current, current->mm->pgd);
    else DBG("Current task %p has no mm!", current);
    for (i = 0; i < DUMPSIZE; i += 0x10) {
        unsigned char buf[0x10];
        memcpy(buf, current->mm->pgd+i, 0x10);
        DBG("%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x",
                buf[0], buf[0+1], buf[0+2], buf[0+3], buf[0+4], buf[0+5], buf[0+6], buf[0+7],
                buf[0+8], buf[0+9], buf[0+10], buf[0+11], buf[0+12], buf[0+13], buf[0+14], buf[0+15]);
    }
    DFS(&init_task, 0);
    unsigned int ttbr0;
    unsigned int ttbr1;
    unsigned int ttbrc;
    asm("mrc p15, 0, %[result], c2, c0, 0" : [result] "=r" (ttbr0) );
    asm("mrc p15, 0, %[result], c2, c0, 1" : [result] "=r" (ttbr1) );
    asm("mrc p15, 0, %[result], c2, c0, 2" : [result] "=r" (ttbrc) );
    DBG("TTBR[0] = %08x TTBR[1] = %08x TTBR_Control = %08x", ttbr0, ttbr1, ttbrc);
    for (i = 0; i < DUMPSIZE; i += 0x10) {
        unsigned char buf[0x10];
        memcpy(buf, 0xffff0000+i, 0x10);
        DBG("%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x",
                buf[0], buf[0+1], buf[0+2], buf[0+3], buf[0+4], buf[0+5], buf[0+6], buf[0+7],
                buf[0+8], buf[0+9], buf[0+10], buf[0+11], buf[0+12], buf[0+13], buf[0+14], buf[0+15]);
    }
